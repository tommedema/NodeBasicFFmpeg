...
inputStream
    .on('data', function(chunk) {
        if (!processor.options.inputStream.write(chunk)) { //write chunk to ffmpeg processor, returns false if kernel buffer is full
            inputStream.pause(); //pause parsing and incoming data while we cannot send more to the receiving end
            processor.options.inputStream.once('drain', function() {
                inputStream.resume();
            }); //resume when kernel buffer is empty again and we can write to receiving end
        }
    })
    .on('end', function () {
