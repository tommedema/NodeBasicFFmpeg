//NOTE: THIS EXAMPLE REQUIRES LIBMP3LAME TO BE INSTALLED AND FFMPEG TO BE CONFIGURED WITH IT
...
var outputStream = 
    fs.createWriteStream('./outputSong.mp3')
    .on('error', function (exception) {
...
var processor = 
    ...
    .on('inputAudioCodec', function (codec) {
        ...
        //bypass if output == input
        if (codec == processor.options.arguments['-f']) {
            util.debug('bypassing because output == input!');
            if (processor.bypass) {
                processor.bypass();
            }
            else {
                util.debug('cannot bypass because not set');
            }
        }
    })
